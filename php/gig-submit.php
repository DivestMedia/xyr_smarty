 ...
 ** ******************************** **/
	...
	// Check Action First!
	if(isset($_POST['action']) && $_POST['action'] == 'gigs_send') {
		$email_body = null;
		$array 		= $required = array();

		// catch post data
		$post_data 	= isset($_POST['gigs']) ? $_POST['gigs'] : null;
		...
		foreach($post_data as $key=>$value) {
			...
			*/
			...
			if(isset($post_data[$key]['required'])) {
				...
				//pass to array
				$array[$key] = $post_data[$key]['required'];
				
				
				
				// We need the user email&name for AddReplyTo on phpmailer!
				if($key == 'full_name')
					$array['full_name'] = $post_data[$key]['required'];
			...
			// ----- extract non-required -----
				...
				//pass to array
				$array[$key] = $post_data[$key];
				

				// We need the user email&name for AddReplyTo on phpmailer!
				if($key == 'full_name')
					$array['full_name'] = $post_data[$key];
		...
		// ATTACHMENT
		if(isset($_FILES['gigs']['name']['attachment']) && $_FILES['gigs']['name']['attachment'] != '') { // name|type|tmp_name|error|size|

			$target_file 	= UPLOAD_FOLDER . basename($_FILES['gigs']['name']['attachment']);
			$rename 		= time() . '_' . basename($_FILES['gigs']['name']['attachment']);
			$uploadOk 		= 1;
			$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);

			// Check if image file is a actual image or fake image
			if(isset($_POST["submit"])) {
				$check = getimagesize($_FILES['gigs']['tmp_name']['attachment']);
				if($check !== false) {
			...
			// Check file size
			if ($_FILES['gigs']['size']['attachment'] > UPLOAD_MAX_SIZE) {
			...
			// if everything is ok, try to upload file
				...
				if (move_uploaded_file($_FILES['gigs']['tmp_name']['attachment'], UPLOAD_FOLDER . $rename)) {

					$email_body .= '<b>Attachment:</b> <a href="' . UPLOAD_FOLDER_URL . $rename . '">'. "{$rename}</a> <br>";
					
					//$array['poster'] = UPLOAD_FOLDER_URL . $rename;
					$array['poster'] = $rename;
				} else {
		...
		// Check File for poster
		if($uploadOk == 0) {
			if($is_ajax === false) {
				_redirect('#alert_mandatory_file');
				exit;
			} else {
				die('_mandatory_');
			}
		...
		if(!$captcha){
          if($is_ajax === false) {
				_redirect('#alert_mandatory_captcha');
				exit;
			} else {
				die('_mandatory_');
			}
	...
	function save_to_db($array){
		
		$username = "gigsmanila_gigs";
		$password = "8rJE4KgAL3";
		$hostname = "localhost"; 
		$db_name = "gigsmanila_gigs"; 
		...
		$_gig_date = date("Y-m-d", strtotime( $array['gig_datetime']));
		
		$ip = ip();
		
		$_data = "INSERT INTO `$db_name`.`wp_gig_submit` 
			(`gig_name`, `gig_description`, `gig_performer`, `gig_venue`, `gig_venue_city`, 
				`gig_date`, `gig_time`, `gig_price`, `poster_url`, 
				`full_name`, `email_ad`, `phone_no`, `user_ip`, `gig_stat`, `date_added`) 
				
		VALUES ('$array[gig_name]', '$array[gig_description]', '$array[gig_performer]', '$array[gig_venue]', '$array[gig_venue_city]', 
				'$array[gig_datetime]', '$array[gig_datetime]', '$array[price]', '$array[poster]', 
				'$array[full_name]', '$array[email]', '$array[phone_no]', '$ip', 'To Review', NOW()); ";
		
		$res = $mysqli->query($_data);
/* 
